    ...
    >
      <div class="searchDiv">
        ...
        <div class="searchItem" v-for="item in searchList" :key="item.id" @click="showItem(item.name)">
          ...
          <div>
            <p v-html="item.name.replace(value, `<font style='color:red'>${value}</font>`)"></p>
...
export default {
  ...
  mounted() {
    ...
    // console.log(PinyinMatch.match('xxx', 'x'));
    console.log(PinyinMatch.default);

    console.log(PinyinMatch.match);
    // 把城市data赋值
  ...
  watch: {
    // 搜索城市
    cityValue(v) {
      console.log(this.PinyinMatch.match);
      let _this = this
      this.searchCity = [];
      for (let item in city.data.cities) {
        city.data.cities[item].map((i) => {
          if (v.trim() !== "" && _this.PinyinMatch.match(i.name, v)) {
