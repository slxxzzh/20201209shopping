      ...
      <div class="Cwrapper" v-if="cityValue.length === 0">
        <div class="CwrapperContent">
          <van-cell-group title="当前城市">
            <van-cell>
              <span @click="replaceCity(this.address)" class="citySpan">{{ this.address }}</span>
            </van-cell>
          </van-cell-group>
          <van-cell-group left title="热门城市">
            <van-cell>
              <span @click="replaceCity(item.name)" v-for="item in hotCities" :key="item.id" class="citySpan">{{
                item.name
              }}</span>
            </van-cell>
          </van-cell-group>
