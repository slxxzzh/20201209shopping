      ...
      <div class="Cwrapper">
        <div class="CwrapperContent">
          ...
          <van-cell-group left title="热门城市">
            <van-cell>
              <span v-for="(item,index) in hotcity" :key="index" class="citySpan">{{item}}</span>
            </van-cell>
          </van-cell-group>
          <van-index-bar>
            <!-- <van-index-anchor v-for="(item,index) in city.data.cities" :key="index" :index="A" /> -->
            <!-- {{item}} -->
            <!-- <van-cell title="文本1" />
            <van-cell title="文本2" />
            <van-cell title="文本3" /> -->
...
export default {
  ...
  mounted() {
    console.log(city.data.cities.A)
    document
