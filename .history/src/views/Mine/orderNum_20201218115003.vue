        ...
        <div class="wrapper" v-if="list1.length!==0">
          <div style="padding:1px 0 110px">
            <div class="listDiv" v-for="item in list1" :key="item._id">
              <h1>订单编号：{{ item.order_id }} <span>{{listStatus(item.status)}}</span></h1>
              <div
          ...
          <div style="padding:1px 0 110px">
            <div class="listDiv" v-for="item in list4" :key="item._id">
              ...
              >
                ...
                <van-button type="warning" @click="$router.push({name:'evaluate',query:{id:JSON.stringify(item)}})" size="small">去评价</van-button>
              </div>
            </div>
          </div>
        </div>
